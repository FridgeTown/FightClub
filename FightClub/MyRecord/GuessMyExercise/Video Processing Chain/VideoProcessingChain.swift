...
struct VideoProcessingChain {
    ...
    // 동작 감지 상태를 관리하는 클래스
    private class ActionDetectionState {
        var lastPunchDetectionTime: Date = .distantPast
        let punchCooldown: TimeInterval = 0.3
        
        func canDetectPunch(at currentTime: Date) -> Bool {
            return currentTime.timeIntervalSince(lastPunchDetectionTime) >= punchCooldown
        }
        
        func updatePunchDetection() {
            lastPunchDetectionTime = Date()
        }
    }

    private let actionState = ActionDetectionState()
...
//               forName: Notification.Name("PunchDetected"),
//               object: nil,
//               queue: .main
//           ) { [weak self] notification in // self를 weak로 캡처
//               guard let self = self else { return } // self가 nil이 아닐 때만 실행
//               if let count = notification.userInfo?["count"] as? Int {
//                   self.delegate?.videoProcessingChain(
//                       self,
//                       didDetectAction: "펀치 \(count)회"
//                   )
//               }
//           }
       }
...
extension VideoProcessingChain {
    ...
    private func findPosesInFrame(_ frame: CGImage) -> [Pose]? {
        ...
        if let observation = humanBodyPoseRequest.results?.first as? VNHumanBodyPoseObservation {
            let pose = Pose(observation)
            
            // 쿨다운 확인 후 펀치 감지
            if actionState.canDetectPunch(at: Date()) {
                if pose!.detectPunchAction(from: observation) {
                    DispatchQueue.main.async {
                        print("펀치 동작이 감지되었습니다!")
                        self.delegate?.videoProcessingChain(self, didDetectAction: "펀치 감지")
                    }
                    actionState.updatePunchDetection()
                }
            }
        }
