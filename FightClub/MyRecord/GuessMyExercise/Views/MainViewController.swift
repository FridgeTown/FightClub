...
class MainViewController: UIViewController {
    ...
    private let punchCooldown: TimeInterval = 0.5  // 1초 쿨다운
...
extension MainViewController: VideoProcessingChainDelegate {
    func videoProcessingChain(_ chain: VideoProcessingChain, didDetectAction action: String) {
        ...
        // 마지막 펀치 감지 이후 쿨다운 시간이 지났는지 확인
        if currentTime.timeIntervalSince(lastPunchTime) >= punchCooldown {
            // 펀치 카운트 증가
            punchCount += 1
            
            // UI 업데이트
            DispatchQueue.main.async { [weak self] in
                // 펀치 카운트를 표시하는 레이블 업데이트
                self?.updatePunchCount(self?.punchCount ?? 0)
            }
    ...
    // 펀치 카운트를 업데이트하는 메서드
    private func updatePunchCount(_ count: Int) {
        // 여기에 펀치 카운트를 표시하는 UI 업데이트 코드 추가
        self.actionLabel.text = "펀치횟수: " + String(count)
    }
